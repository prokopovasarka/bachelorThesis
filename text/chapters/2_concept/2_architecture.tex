...
koncept Component-base architektury relevantní.

\subsection{Výhody a nevýhody zmíněných architektur}
Po stručném popsání jednotlivých architektur a návrhových vzorů je třeba před závěrečným výběrem shrnout si
jejich výhody a nevýhody. Nejprve následuje popis \textbf{výhod} plynoucích z využívání jednotlivých architektur.
...
\paragraph{MVVM}
Architektura MVVM není vhodná architektura pro projekty s jednoduchým UI vzhledem k jeho komplexnosti. Zároveň je náročné správně navrhnout ViewModel, což by 
mohlo vyústit k vysokému počtu bloků s duplicitním kódem.~\cite{gossman2006advantages}

\paragraph{Component-based}
Vzhledem k vytváření oddělených komponent, Component-based architektura přidává do systému vyšší komplexitu. Každá její komponenta musí být přesně definována, 
...
\subsection{Shrnutí a výběr architektury}
Po shrnutí výhod a nevýhod všech zmíněných architektur jsme získali lepší obrázek o tom, jak každá funguje a k jakému účelu slouží. Nyní proto můžeme vybrat 
architekturu nejvhodnější pro náš projekt.
...
MVC je sice vhodný pro projekty se stejnými rozměry, jako má tato práce, avšak vhodnější volbou je Component-based architektura, a to z několika následujících důvodů. MVC často vyžaduje explicitní instrukce
při změně modelu pro aktualizaci View, zároveň s růstem aplikace začíná být zajišťování závislostí a udržování oddělení částí mnohem náročnější. Component-based architektura je založena na dynamičnosti
...
Dle~\cite{reactnative_components} je React Native založený na komponentách a kromě základních a nativních komponent existují i další, vytvořené komunitou. Můžeme 
tedy říci, že pro zájmy této práce a v souladu se standardními postupy v softwarovém inženýrství byla vybrána správná architektura pro vyvíjenou aplikaci a můžeme pokračovat k poslední části návrhu.