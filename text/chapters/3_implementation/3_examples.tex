...
Sekce je tedy dle využitého vzoru Component-based rozdělená na popis jednotlivých komponent důležitých pro fungování celé 
aplikace.
...
již spuštěna, dle této podmínky přesměruje uživatele na \texttt{Help} či \texttt{HomeScreen}. (Viz Ukázka \ref{fig:first_launch})
...
\end{minted}
\noindent\textit{Ukázka implementace hooks při prvním spuštění.}\mbox{}\\
...
\texttt{react-native-pixel-color} lze získat HEX kód barvy. Následně aktualizuje status. (Viz Ukázka \ref{fig:handle_tap})
...
\end{minted}
\noindent\textit{Ukázka implementace detekce barvy kliknutím.}\mbox{}\\
\label{fig:handle_tap}
...
přístup do knihovny, kam se fotografie ukládá. (Viz Ukázka \ref{fig:safe_photo})
...
\end{minted}
\caption{Ukázka implementace ukládání fotografie.}
\label{fig:safe_photo}
\end{figure}
...
\texttt{react-native-image-picker}. Pro načtení využívá časovač, během kterého se nastaví vybraná fotografie. (Viz Ukázka \ref{fig:pick_gallery})
...
\end{minted}
\noindent\textit{Ukázka implementace funkce výběru fotografie z kamery.}\mbox{}\\
\label{fig:pick_gallery}
...
AR objektu do prostoru. V něm, jak je vidět v Ukázce \ref{fig:hit_test} níže, využívá hit test \texttt{performARHitTestWithPoint} vysílající paprsek dle souřadnic bodu pixelu v 2D scéně.
...
\end{minted}
\noindent\textit{Ukázka implementace hit testu.}\mbox{}\\
\label{fig:hit_test}
...
převodu z HEX do CMYK. Nejprve dojde k separaci jednotlivých složek, následně proběhne převod. (Viz Ukázka \ref{fig:hex_to_cmyk})
...
\end{minted}
\noindent\textit{Ukázka implementace funkce převodu kódů.}\mbox{}\\
\label{fig:hex_to_cmyk}
...
Druhá část komponenty je určena pro tvorbu palet. Následující ukázka \ref{fig:generating_palettes} zobrazuje celý postup výpočtu monochromatické, analogické i komplementární 
palety. U monochromatické palety byl zvolen offset, díky kterému je zajištěn při jasných či naopak velmi tmavých odstínech posun, aby paleta 
nevyobrazila pouze černé či pouze bílé odstíny.
